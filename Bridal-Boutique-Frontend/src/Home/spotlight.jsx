...
import api, { resolveMediaUrl, FALLBACK_IMAGE } from "../services/api";   // adjust path
// If FALLBACK_IMAGE is not needed, you can omit it and use a hardcoded fallback
...
const Spotlight = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // ✅ Use api.get – base URL is prepended automatically
      const bannerUrl = `/banner/get_banner.php?banner_title=${encodeURIComponent(bannerTitleParam)}`;
      let productUrl = `/product/get.php?limit=2`;
      if (companyId) {
  ...
  // ✅ No need for local resolveImage / resolveAsset – use resolveMediaUrl directly
  const featuredProduct = featuredProducts[0];

  // Debug – optional
  console.log("spotlight bannerData:", bannerData);

  // ✅ Use resolveMediaUrl for banner image, fallback if missing
  const bannerImage = resolveMediaUrl(bannerData?.image) || 
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {featuredProducts.map((product) => (
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden">
              {product.video_url ? (
                <video
                  src={resolveMediaUrl(product.video_url)}   // ✅ unified resolver
                ...
                <img
                  src={resolveMediaUrl(product.image)}      // ✅ unified resolver
        ...
        >
          ...
          <div className="absolute bottom-12 left-10">
            ...
            <p className="mt-4 text-white text-lg">{bannerSubtitle}</p>
